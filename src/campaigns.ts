 ...
 *
 * 约定：
 *   <name>.csv           内容 CSV（原样上传到 Meta 的 Create from CSV）
 *   <name>.schedule.csv  排期表（label,date,send_time_strategy）
...
export function discoverCampaigns(dir: string): ResolvedGameJob[] {
  ...
  for (const csv of contentCsvs.sort()) {
    ...
    const scheduleAbs = join(dirAbs, scheduleFile);

    if (!existsSync(scheduleAbs)) {
      log.warn(`跳过「${csv}」：缺少配套排期表「${scheduleFile}」。`);
      continue;
    }

    const notifications = parseScheduleSheet(scheduleAbs);
    const { projectName, url } = resolveProjectInfo(name, projectMap[name]);
    jobs.push({ projectName, url, csv: join(dirAbs, csv), notifications });
    log.info(
      `发现 campaign：${csv} -> 项目「${projectName}」${url ? '（直达 URL）' : ''}，` +
        `${notifications.length} 条推送（排期表 ${scheduleFile}）`,
    );
  }
  return jobs;
}
