 ...
 */
export function validateContentCsv(
  csvPath: string,
  ...
  try {
    ...
    return { errors: [`无法读取内容表: ${abs}（${(e as Error).message}）`], warnings, rowCount: 0, labels: [] };
  }
  ...
  try {
    ...
    return { errors: [`内容表不是合法 CSV: ${abs}（${(e as Error).message}）`], warnings, rowCount: 0, labels: [] };
  }
  ...
  const unsupported = namedCols.filter(
    (c) => !SUPPORTED_BASE_COLUMNS.has(c) && !c.startsWith(TITLE_PREFIX) && !c.startsWith(BODY_PREFIX),
  );
  if (unsupported.length > 0) {
    warnings.push(`存在 Meta 不支持的列（会被忽略）：${unsupported.join(', ')}。图片列请用 media_url。`);
  }
  ...
  rows.forEach((row, idx) => {
    ...
    if (hasLabel) {
      if (!label) {
        ...
        if (prev !== undefined) {
          errors.push(`label 重复：「${label}」出现在第 ${prev} 行和第 ${lineNo} 行（重复 label 会导致整批 0 created）。`);
        } else {
    ...
    if (pairedLangs.length > 0) {
      const ok = pairedLangs.some(
        (l) => (row[`${TITLE_PREFIX}${l}`] ?? '').trim() !== '' && (row[`${BODY_PREFIX}${l}`] ?? '').trim() !== '',
      );
      if (!ok) {
        errors.push(`第 ${lineNo} 行 (${label || '无 label'}) 没有任何一种语言同时填了 title 和 body。`);
      }
    }
